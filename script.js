...
(function () {
  ...
  function gestionarIntro() {
    ...
    var duracion = reduceMov ? 900 : 3300;
  ...
  function gestionarMenuMovil() {
    ...
    // Cerrar si se pasa a la navegación horizontal de escritorio
  ...
  function construirMenuPremium(links) {
    ...
    var S = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">';
    var iconos = {
    ...
    var flecha = '<svg class="mi-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M9 6l6 6-6 6"/></svg>';
  ...
  /* -------------------------------------------------------------------
     8. BARRA DE PROGRESO DE LECTURA
  ------------------------------------------------------------------- */
  ...
  /* -------------------------------------------------------------------
     9. CONTADORES ANIMADOS (cuenta ascendente al ver la métrica)
     Marca el número con data-count (y opcional data-decimals/prefix/suffix).
  ------------------------------------------------------------------- */
  function gestionarContadores() {
    var els = document.querySelectorAll("[data-count]");
    if (!els.length) return;

    var reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

    function pinta(el, val) {
    ...
    function animar(el) {
      var objetivo = parseFloat(el.getAttribute("data-count"));
      if (reduce) { pinta(el, objetivo); return; }
      var dur = 1200, t0 = performance.now();
      function frame(now) {
        var p = Math.min((now - t0) / dur, 1);
        var eased = 1 - Math.pow(1 - p, 3);
        pinta(el, objetivo * eased);
    ...
    if (!("IntersectionObserver" in window)) {
      els.forEach(function (el) { animar(el); });
      return;
    }
    var obs = new IntersectionObserver(function (entradas) {
